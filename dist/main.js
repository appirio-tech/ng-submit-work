...
$templateCache.put("views/submit-work-type.directive.html","<header><h1>How to create a new project</h1><p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p></header><form ng-submit=\"vm.create()\"><div flush-height=\"flush-height\" class=\"flush-height flex column\"><div class=\"interactive flex column center flex-grow\"><input type=\"text\" placeholder=\"Name your project...\" required=\"true\" ng-model=\"vm.name\" class=\"wider error\"/><p class=\"error\">The app name has been taken, please try another.</p></div><button type=\"button\" scroll-element=\"platform-details\" class=\"wider continue action\">continue</button></div><div full-height=\"full-height\" class=\"full-height flex column\"><div id=\"platform-details\" class=\"interactive flex column center flex-grow\"><h2>IOS <strong>platform details</strong></h2><ul class=\"target-platforms\"><li><h4>Devices</h4><hr/><ul><li ng-repeat=\"device in vm.devices\"><checkbox label=\"{{device.name}}\" ng-model=\"device.selected\"></checkbox></li></ul></li><li><h4>Orientation</h4><hr/><ul><li ng-repeat=\"orientation in vm.orientations\"><checkbox label=\"{{orientation.name}}\" ng-model=\"orientation.selected\"></checkbox></li></ul></li></ul><p class=\"error\">Please choose a device.</p><p class=\"error\">Please choose an orientation.</p></div><button type=\"button\" scroll-element=\"type-details\" class=\"wider continue action\">continue</button></div><div full-height=\"full-height\" class=\"full-height flex column\"><div id=\"type-details\" class=\"interactive flex column center flex-grow\"><h2>What <strong>type of work</strong> are you looking for?</h2><ul class=\"type flex-center\"><li ng-repeat=\"projectType in vm.projectTypes\"><h4>{{projectType.name}}</h4><img src=\"{{ projectType.imgUrl }}\"/><p>{{ projectType.description }}</p><button type=\"button\" selectable=\"true\" ng-model=\"vm.projectType\" value=\"projectType.id\"></button></li></ul><p class=\"error\">Please choose a type of work.</p></div><button type=\"button\" scroll-element=\"brief-details\" class=\"wider continue action\">continue</button></div><div full-height=\"full-height\" class=\"full-height flex column has-loader\"><loader ng-if=\"vm.loading\"></loader><div id=\"brief-details\" class=\"interactive flex column center flex-grow\"><h2>Can you <strong>share a brief</strong> overview?</h2><textarea placeholder=\"E.g. I need a mobile HR application with social features to support my growing organization\" ng-model=\"vm.brief\" class=\"brief\"></textarea><button type=\"submit\" class=\"action continue wider\">Create Project</button></div></div></form><modal show=\"vm.showSuccessModal\" background-click-close=\"background-click-close\"><div class=\"success\"><h2>Awesome!</h2><p>Your {{ vm.name }} is set up now</p><p>Share your email to signup and we\'ll be sure to send a project link.</p><form><input type=\"email\" required=\"required\" class=\"wider\"/><button type=\"submit\" class=\"wider\">Submit</button></form></div></modal>");
...
(function() {
  ...
  SubmitWorkService = function($rootScope, OptimistModel, SubmitWorkAPIService, SubmitWorkCreationAPIService) {
    ...
    create = function(updates) {
      ...
      apiCall = function(model) {
        return SubmitWorkCreationAPIService.post({}, model).$promise.then(interceptResponse);
      };
      return work.update({
        updates: updates,
        apiCall: apiCall
